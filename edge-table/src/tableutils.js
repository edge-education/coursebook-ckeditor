...
export default class TableUtils extends Plugin {
        ...
        model.change((writer) => {
            ...
            for (const { row, column, cellHeight, cellWidth, cell } of tableIterator) {
                ...
                // Store the colspan from reference row.
                    ...
                    for (let i = 0; i < cellWidth; i++) attrs.push(cell.getAttributes());
                    rowColSpansMap[column] = cellWidth;
                }
            }
            for (let rowIndex = 0; rowIndex < rowsToInsert; rowIndex++) {
                ...
                for (let cellIndex = 0; cellIndex < rowColSpansMap.length; cellIndex++) {
                    ...
                    if (colspan > 0) {
                        if (colspan > 1) attrs[cellIndex].colspan = colspan;

                        createEmptyTableCell(writer, insertPosition, attrs[cellIndex]);
                    }
     ...
     * @param insertBefore a boolean to determine if before or after
     */
    insertColumns(table, options = {}, insertBefore = null) {
        const model = this.editor.model;
        const insertAt = options.at || 0;
            ...
            for (const tableSlot of tableWalker) {
                ...
                if (cellAnchorColumn < insertAt) {
                    ...
                    createCells(
                        ...
                        insertBefore ? cell.getAttributes() : cell.previousSibling.getAttributes()
                    );
                }
            }
        });
...
function adjustHeadingColumns(table, removedColumnIndexes, writer) {
    ...
    if (headingColumns && removedColumnIndexes.first < headingColumns) {
        const headingsRemoved =
            Math.min(headingColumns - 1 /* Other numbers are 0-based */, removedColumnIndexes.last) -
