...
import ListBracketedCommand from './listbracketedcommand';
...
function createAttributeStrategies(enabledProperties) {
    ...
    if (enabledProperties.bracketed) {
        strategies.push({
            ...
            defaultValue: false,
            addCommand(editor) {
                editor.commands.add('listBracketed', new ListBracketedCommand(editor));
            },
            appliesToListItem(item) {
                return item.getAttribute('listType') == 'numbered';
            },
            setAttributeOnDowncast(writer, listBracketed, element) {
                if (listBracketed) {
                    writer.setAttribute('bracketed', 'bracketed', element);
                } else {
                    writer.removeAttribute('bracketed', element);
                }
            },
            getAttributeOnUpcast(listParent) {
...
function downcastListItemAttributes(attributeStrategies) {
    ...
    function areRepresentingSameList(listItem1, listItem2) {
        return listItem2 &&
            ...
            listItem1.getAttribute('listBracketed') === listItem2.getAttribute('listBracketed') &&
...
function removeListItemAttributesFromTodoList(editor) {
    return (writer) => {
        const todoListItems = getChangedListItems(editor.model.document.differ.getChanges())
            .filter(item => {
                ...
                return item.getAttribute('listType') === 'todo' && (item.hasAttribute('listStyle') ||
                    ...
                    item.hasAttribute('listBracketed') ||
        ...
        for (const item of todoListItems) {
            ...
            writer.removeAttribute('listBracketed', item);
        }
        return true;
    };
}
