...
import { FocusTracker, global, KeystrokeHandler } from 'ckeditor5/src/utils';
...
export default class ListPropertiesView extends View {
    ...
    constructor(locale, { enabledProperties, styleButtonViews, styleGridAriaLabel }) {
        ...
        /**
         * A switch button allowing the user to make the edited list bracketed.
         *
         * **Note**: Only present when the view represents **numbered** list properties.
         *
         * @readonly
         */
        this.bracketedSwitchButtonView = null;
        /**
    ...
    render() {
        ...
        if (this.stylesView) {
            ...
            if (this.startIndexFieldView || this.reversedSwitchButtonView || this.bracketedSwitchButtonView) {
        ...
        if (this.bracketedSwitchButtonView) {
            this.focusables.add(this.bracketedSwitchButtonView);
            this.focusTracker.add(this.bracketedSwitchButtonView.element);
        }
    ...
    _addNumberedListPropertyViews(enabledProperties) {
        ...
        if (enabledProperties.bracketed) {
            this.bracketedSwitchButtonView = this._createBracketedSwitchButton();
            numberedPropertyViews.push(this.bracketedSwitchButtonView);
        }
    ...
    /**
     * Creates the bracketed list switch button.
     */
    _createBracketedSwitchButton() {
        const bracketedButtonView = new SwitchButtonView(this.locale);
        bracketedButtonView.set({
            withText: true,
            label: 'Bracketed Markers',
            class: 'ck-numbered-list-properties__bracketed'
        });
        bracketedButtonView.delegate('execute').to(this, 'listBracketed');
        return bracketedButtonView;
    }
}
